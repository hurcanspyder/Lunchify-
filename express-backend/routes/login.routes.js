...
router.post('/admin/login', async (req, res) => {
  try {
    ...
    const [rows] = await mysqlPool.query(
      'SELECT id, name, password, role, project_id, canteen_id FROM users WHERE id = ?',
      [formattedId]
    );
...
router.post('/upsert-user', requireAuth, requireITAdmin, async (req, res) => {
  try {
    const { employeeId, name, department, phone, password, role, project_id, canteen_id } = req.body;
    ...
    if (rows.length > 0) {
      ...
      let query = `
        UPDATE users 
        SET name = ?, department = ?, phone = ?, role = ?, project_id = ?, canteen_id = ?, is_registered = 1, is_admin = ?, is_active = 1
      `;
      const params = [name, department, phone, finalRoleId, finalProjectId, finalCanteenId, isAdmin];
      ...
      const hashedPassword = await bcrypt.hash(password, 10);
      await mysqlPool.query(
        `INSERT INTO users (id, name, department, phone, password, role, project_id, canteen_id, is_registered, is_admin, is_active, coupons_left, monthly_limit)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 1, 16, 16)`,
        [formattedId, name, department, phone, hashedPassword, finalRoleId, finalProjectId, finalCanteenId, isAdmin]
      );
